...
export default function TeachersPage() {
  ...
  // FIX: Edit form submit now calls updateTeacher mutation
  const onEditSubmit = async (d: any) => {
    if (!editTeacher) return;
    try {
      await updateTeacher({ id: editTeacher._id, data: d }).unwrap();
    ...
    <div className="p-6 space-y-6">
          ...
          <div className="glass w-full max-w-md animate-fade-up max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={createForm.handleSubmit(onCreateSubmit)} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input {...createForm.register('phone')} placeholder="+1 234 567 890" className="input mt-1.5" />
              </div>
      ...
      {editTeacher && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass w-full max-w-md animate-fade-up max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              <h2 className="font-display font-bold text-xl text-text-primary">Edit Teacher</h2>
              <button onClick={() => setEditTeacher(null)} className="p-1.5 rounded-lg text-text-tertiary hover:text-text-primary"><X size={18} /></button>
            </div>
            <form onSubmit={editForm.handleSubmit(onEditSubmit)} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Phone</label>
                <input {...editForm.register('phone')} className="input mt-1.5" />
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setEditTeacher(null)} className="btn-secondary flex-1">Cancel</button>
                <button type="submit" disabled={isUpdating} className="btn-primary flex-1 justify-center">
