...
import { BookOpen, Plus, Clock, CheckCircle, AlertCircle, X, Loader2, Calendar, Users } from 'lucide-react';
...
const { useGetAssignmentsQuery, useCreateAssignmentMutation } = assignmentApi;
...
export default function AssignmentsPage() {
  ...
  const overdueCount = assignments.filter((a: any) => new Date(a.dueDate) < new Date() && a.status !== 'closed').length;
  const pendingSubmissions = assignments.reduce((acc: number, a: any) => acc + (a.pendingCount || 0), 0);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between animate-fade-up">
        <div>
          ...
          <p className="text-text-secondary text-sm">
            {assignments.length} assignments • {overdueCount > 0 && <span className="text-danger">{overdueCount} overdue</span>}
          </p>
        </div>
        {canCreate && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          [...Array(6)].map((_, i) => <div key={i} className="skeleton h-44 rounded-card animate-fade-up" />)
          ...
          <div className="col-span-full card p-16 text-center animate-fade-up">
            ...
            {canCreate && (
              <button onClick={() => setShowCreate(true)} className="text-accent text-sm mt-2 hover:underline">
          ...
          return (
            <div key={a._id} className={`card p-5 space-y-3 animate-fade-up ${isOverdue ? 'border-danger/20' : ''}`}
              style={{ animationDelay: `${i * 0.05}s` }}>
              <div className="flex items-start justify-between">
                <span className={`badge ${typeColors[a.type] || 'badge-neutral'} text-xs`}>{a.type}</span>
                {isOverdue ? (
                  <span className="flex items-center gap-1 text-xs text-danger"><AlertCircle size={11} />Overdue</span>
                ) : (
                  <span className="flex items-center gap-1 text-xs text-text-tertiary"><Clock size={11} />{dueText}</span>
                )}
              </div>
